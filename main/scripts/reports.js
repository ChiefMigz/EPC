...
async function onCreatePageTypeSelectorButtonClick(type) {
  ...
  // Update save button text based on report type
  const saveButton = document.querySelector('.createPage .saveButton')
  if (type === 'citation') {
    saveButton.textContent = 'Print Ticket'
  } else if (type === 'arrest') {
    saveButton.textContent = 'Print Report'
  } else {
  ...
  // If there is a session copy saved locally (from settings save), prefer those values
  try {
    const sessionCopy = JSON.parse(localStorage.getItem('officerInformationSession'))
    if (sessionCopy && typeof sessionCopy === 'object') {
      // merge sessionCopy over server values so most-recent-session wins
      Object.assign(officerInformation, sessionCopy)
    }
  } catch (e) {
