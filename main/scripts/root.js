...
async function openPedInReport(type, pedName = '', vehiclePlate = '') {
  ...
  iframe.onload = async () => {
    ...
    // Fill ped name
    const nameInput = iframe.contentWindow.document.querySelector('.createPage #offenderSectionPedNameInput');
    if (nameInput) nameInput.value = pedName;
          // Fill vehicle plate from localStorage database after short delay
          setTimeout(() => {
            let plateDb = JSON.parse(localStorage.getItem('plateDb') || '[]');
            let latestPlate = plateDb.length > 0 ? plateDb[plateDb.length - 1].plate : vehiclePlate;
            const plateInput = iframe.contentWindow.document.querySelector('.createPage #offenderSectionVehicleLicensePlateInput');
            if (plateInput && latestPlate) plateInput.value = latestPlate;
          }, 300);
  }
}
...
// Ensures the CSS variable --header-height matches the actual header height
// so fixed desktop content is positioned correctly beneath it.
...
function updateHeaderCssVariable() {
  try {
    const header = document.querySelector('.mdt-header, .lapd-header')
    if (!header) return
    ...
    // harmless if running in contexts without DOM
    // keep silent — nothing to do
  }
}

const debouncedUpdateHeader = debounce(updateHeaderCssVariable, 120)

if (typeof window !== 'undefined' && typeof document !== 'undefined') {
  document.addEventListener('DOMContentLoaded', () => {
    updateHeaderCssVariable()

    // update on resize/orientation changes
    window.addEventListener('resize', debouncedUpdateHeader)

    // observe header mutations (content changes that may alter height)
    const headerEl = document.querySelector('.mdt-header, .lapd-header')
    if (headerEl && window.MutationObserver) {
      const mo = new MutationObserver(debouncedUpdateHeader)
      mo.observe(headerEl, {
        childList: true,
        subtree: true,
        attributes: true,
